...
public class RoughKeyboard {
    ...
    대충 만든 자판
        ...
        예를 들어, 1번 키에 "A", "B", "C" 순서대로 문자가 할당되어 있다면 1번 키를
        한 번 누르면 "A", 두 번 누르면 "B", 세 번 누르면 "C"가 되는 식입니다.
    ...
    public int[] solution(String[] keymap, String[] targets) {
        ...
        for (String keys : keymap) {
            for (int i = 0; i < keys.length(); i++) {
                ...
                char oneAlphabet = keys.charAt(i);

                int step = map.getOrDefault(oneAlphabet, i + 1);

                map.put(oneAlphabet, Math.min(step, i + 1));
            }

            for (int k = 0; k < targets.length; k++) {

                for (int j = 0; j < targets[k].length(); j++) {
                    char t = targets[k].charAt(j);
