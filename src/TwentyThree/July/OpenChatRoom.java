...
public class OpenChatRoom {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(openChatRoom.solution(record)));
    }

    public String[] solution(String[] record) {
        HashMap<String, String> userIdAndNickNameMap = new HashMap<>();
        String changeEnter = "님이 들어왔습니다.";
        String changeLeave = "님이 나갔습니다.";
        int listLength = record.length;
        List<String> changeList = new ArrayList<>();
        String[] splitStr;
        for (String str : record) {
            splitStr = str.split(" ");
            if(str.charAt(0) == 'L')  continue;
            if(str.charAt(0) == 'C') listLength--;
            userIdAndNickNameMap.put(splitStr[1], splitStr[2]);
        }

        String[] answer = new String[listLength];
        int idx = 0;
        for (String str : record) {
            splitStr = str.split(" ");
            if(str.charAt(0) == 'E') {
                answer[idx++] = userIdAndNickNameMap.get(splitStr[1]) + changeEnter;
            }else if(str.charAt(0) == 'L') {
                answer[idx++] = userIdAndNickNameMap.get(splitStr[1]) + changeLeave;
            }
        }
